...
public class SimpleTypedGrokPatternBenchmark {

    @State(Scope.Thread)
    public static class BenchmarkState {
        private static final Map<String, String> PATTERNS = new HashMap<>();
        private static final String SEARCH_PATTERN_ES = "%{DATA} %{NUMBER:my_number:int}";
        private static final String SEARCH_PATTERN_THEKRAKKEN = "%{DATA} %{NUMBER:my_number;int}";
        ...
        public void doSetup() throws GrokException {
            grokProcessor = new GrokProcessor("", PATTERNS, Collections.singletonList(SEARCH_PATTERN_ES), "field", false, true);

            grok = new Grok();
            PATTERNS.forEach((name, pattern) -> silentlyAddGrokPattern(grok, name, pattern));
            grok.compile(SEARCH_PATTERN_THEKRAKKEN);
        }
    ...
    @Benchmark
    public void testElasticsearchGrokProcessorSimpleTypedMatch(BenchmarkState state, Blackhole bh) throws Exception {
    ...
    @Benchmark
    public void testTheKrakkenGrokSimpleTypedMatch(BenchmarkState state, Blackhole bh) {
